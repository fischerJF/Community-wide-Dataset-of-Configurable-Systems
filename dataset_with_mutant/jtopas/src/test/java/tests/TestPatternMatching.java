...
public class TestPatternMatching extends JTopasTest {
  ...
  /**
   * Test the case, when a line comment is not terminated by a newline
   * character. This happens when the last line of a file is a line comment
   * without a newline on its end. This is a rather common situation.
   */
...
//  @Test
  public void compareJTopasAgainstPattern() throws Throwable {
    ...
    TokenizerProperties jtopasProps = new StandardTokenizerProperties(
        Flags.F_RETURN_WHITESPACES | Flags.F_TOKEN_POS_ONLY);
    TokenizerProperties patternProps = new StandardTokenizerProperties(
        Flags.F_RETURN_WHITESPACES | Flags.F_TOKEN_POS_ONLY);

    // patternProps.addPattern("/\\*\\*.*\\*/", docComment,
    // Flags.F_FREE_PATTERN);
    ...
    // patternProps.addPattern("\"[[^$\"]|[\\\\&&\"]]*?\"", string,
    // Flags.F_FREE_PATTERN);
    patternProps.addPattern("'.'", character, Flags.F_FREE_PATTERN);
    // patternProps.addPattern("'[[^']|[\\\\&&']]+?'", character,
    // Flags.F_FREE_PATTERN);
   ...
   */
  private void tokenize(TokenizerProperties props, String data)
      throws Throwable {
