...
/**
 * This is the DeflaterHuffman class. This class is <i>not</i> thread safe. This
 * is inherent in the API, due to the split of deflate and setInput.
 * 
  ...
  class Tree {
    ...
    if (Configuration.BASE){
        ...
        do {
          ...
          int mindepth = Math.min(values[first] & 0xff,
              values[second] & 0xff);
          values[last] = lastVal = values[first] + values[second]
              - mindepth + 1;
          ppos = 0;
          path = 1;
          while (path < heapLen) {
