...
public abstract class AbstractTokenizer implements Tokenizer, TokenizerPropertyListener {
	...
	public void setSource(TokenizerSource source) {
		...
		_currentReadPos   = 0;
		_currentWritePos  = 0;
		if (Configuration.COUNTLINES){ //<(isFlagSet(Flags.F_COUNT_LINES)) {//>
			_lineNumber     = 0;
			_columnNumber   = 0;
		} else {
			_lineNumber     = -1;
		...
		__MAIN_LOOP__:
			do {
				...
				case Token.BLOCK_COMMENT:
					...
					returnIt = false;
					if(Configuration.BLOCKCOMMENTS)
						returnIt = true;
				...
				if (returnIt) {
					...
					if(Configuration.TOKENPOSONLY) /* (prop != null) ? isFlagSet(prop, Flags.F_TOKEN_POS_ONLY) : */ 
					...
					if(Configuration.IMAGEPARTS)                                           //              isFlagSet(Flags.F_RETURN_IMAGE_PARTS); //>
	...
	protected boolean isPattern(int offset, boolean freePatternOnly) throws TokenizerException {
		if (_patternHandler != null) {
			...
			if (result != null && (isFree || ! freePatternOnly)) {
				...
				if(Configuration.IMAGEPARTS) {  //<chpkim (isFlagSet(result.getProperty(), Flags.F_RETURN_IMAGE_PARTS)) { //>
