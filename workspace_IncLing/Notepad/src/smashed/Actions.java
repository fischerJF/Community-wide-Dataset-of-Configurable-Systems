...
/*@(Persistence)*/
  //for using a open & save dialog
	@Feature___("Persistence")
	protected ExampleFileFilter filter = new ExampleFileFilter();
/*@(Persistence)*/
...
/*@(Persistence)*/
  //to get the text from the text area
	...
	public void neW()
	{
		if (Configuration.PERSISTENCEMENUBAR || Configuration.PERSISTENCETOOLBAR)
		{
			/**
			...
			if(!n.getTextArea().getText().equals("") && !n.getTextArea().getText().equals(fileContent))
			{
				...
				if(fileName == null)
				{
					...
					if(option == 0)
					{
						...
						n.setTitle("Untitled - JAVA(TM) Notepad");
					}
...
/*@(WordCount)*/
	 ...
	 */
	@Feature___("WordCount")
	public void wordCount()
	{
		String[] tokens = n.getTextArea().getText().trim().split("\\s+");
		int count = tokens.length;

		// This is a corner case - an empty program returns 1
		if(count == 1 && tokens[0].isEmpty())
			count = 0;
			
		String message = "Word count: " + count;
