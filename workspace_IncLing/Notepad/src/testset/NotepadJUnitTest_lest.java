...
//		Configuration.EDITTOOLBAR = true;
		...
		GenericTypeMatcher<JButton> but = new GenericTypeMatcher<JButton>(JButton.class) {
			@Override
		...
		JButtonFixture buto = window.button(but);// pesquisando pelo
													// bot??????????????????????????????????????????????????????o
		...
		buto.click();
		// /JOptionPaneFixture dialog = window.optionPane();
		// dialog.requireTitle("Selecionar uma
		// Op????????????????????????????????????????????????????????????????????????????????????????????????????????????o");
...
//		Configuration.EDITTOOLBAR = true;
	...
	//	textArea.requireText("Hello");
		...
		JButtonFixture buto = window.button(but);// pesquisando pelo
													// bot??????????????????????????????????????????????????????o
		buto.click();// clicando no bot??????????????????????????????????????????????????????o
						// selecionado

		// COLAR
		GenericTypeMatcher<JButton> butt = new GenericTypeMatcher<JButton>(JButton.class) {
			@Override
			protected boolean isMatching(JButton button) {
		...
		JButtonFixture buton = window.button(butt);// pesquisando pelo
													// bot??????????????????????????????????????????????????????o
		buton.click();// clicando no bot??????????????????????????????????????????????????????o
						// selecionado
