 ...
 * 
 * Der einfachste ContentHandler: Er liest nur den Dateiinhalt ein und verwendet
 * den Dateinamen ohne Endung als Dateititel.
 * 
 * @author Mr. Pink
 */
public class Plain extends ContentHandler {
	...
	public String[] getIndexedFields() {
		if (DesktopSearcherVariables.getSINGLETON().isTXT()) {
	...
	public boolean index(String filename, IndexWriter writer) {
		if (DesktopSearcherVariables.getSINGLETON().isTXT()) {
			try {
				...
				Field field = new Field("content", plainText + " ",
						Field.Store.YES, Field.Index.ANALYZED,
						Field.TermVector.YES);

				doc.add(field);

				doc.add(new Field("type", "plain", Field.Store.YES,
