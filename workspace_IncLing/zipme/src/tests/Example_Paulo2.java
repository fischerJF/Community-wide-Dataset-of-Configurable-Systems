...
//	  Configuration.EXTRACT=true;
    if(Configuration.GZIP && Configuration.CRC && Configuration.DERIVATIVE_COMPRESS_GZIPCRC &&  Configuration.EXTRACT) {
    ZipInputStream zis = new ZipInputStream(new FileInputStream(new File(
        homeDir + "/files/src.zip")));
    zis.hook();
    assertTrue(zis.available() == 1);
...
//	  Configuration.DERIVATIVE_COMPRESS_GZIPCRC=true;
//    
//    if(Configuration.GZIP && Configuration.CRC && Configuration.DERIVATIVE_COMPRESS_GZIPCRC && Configuration.DERIVATIVE_COMPRESS_GZIPCRC) {
    ZipInputStream zis = new ZipInputStream(new FileInputStream(new File(
        homeDir + "/files/src.zip")));
    zis.hook();
    assertTrue(zis.available() == -1);
